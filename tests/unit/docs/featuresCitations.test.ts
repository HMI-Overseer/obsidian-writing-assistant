...
/**
 * CI guard for FEATURES.md: it is the canonical feature -> source map, so every
 * `src/...` path it cites must exist on disk. Catches the kind of drift where a
 * file is renamed or deleted (e.g. DiffReviewPanel.ts -> DiffHunkView.ts) but the
 * registry keeps pointing at the old path.
 *
...
const featuresPath = resolve(repoRoot, "docs/FEATURES.md");
